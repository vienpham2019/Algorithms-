...
const Block = function(x , y , prev_node = null){
    this.x = x 
    this.y = y 
    this.dx = size
    this.dy = size
...
const setUp = () => {
    ...
    block = new Block(0 , 0)
    start_node = new Block(0 , 0)
    quere.push(start_node)
    end_node = new Block(width - size,height - size)
    draw_maze()
}
...
const draw_maze = () => {
    ...
    if(stack.length === 0){
        start_node.draw("blue")
        end_node.draw("green")
        if(end_node.prev_node){
            cancelAnimationFrame(myReq)
        }else{
            solve_maze()
            for(let i = 0; i < visited_nodes_for_path.length; i ++){
                visited_nodes_for_path[i].draw("yellow")
            }
        }
    }else{ 
        block.move()  
...
const solve_maze = () => {
    ...
    let current_node = quere.shift()
    visited_nodes_for_path.push(current_node)
    console.log(quere)
    console.log(visited_nodes_for_path)
    if(end_node.prev_node){
        return
    }
...
const check_neighbor_node = (node) => {
    ...
    if(top && !top.walls[2] && !visited_nodes_for_path.find(n => n.x === top.x  - (size / 2)&& n.y === top.y + (size / 2))){
        console.log("top")
        ...
        if(top_x === end_node.x && top_y === end_node.y){
            ...
            let top_block = new Block(top_x, top_y, node)
    ...
    if(right && !right.walls[3] && !visited_nodes_for_path.find(n => n.x === right.x - (size / 2) && n.y === right.y - (size / 2))){
        console.log("right")
        ...
        if(right_x === end_node.x && right_y === end_node.y){
            ...
            let right_block = new Block(right_x, right_y, node)
    ...
    if(bottom && !bottom.walls[0] && !visited_nodes_for_path.find(n => n.x === bottom.x - (size / 2) && n.y === bottom.y  - (size / 2))){
        console.log("bottom")
        ...
        if(bottom_x === end_node.x && bottom_y === end_node.y){
            ...
            let bottom_block = new Block(bottom_x, bottom_y, node)
    ...
    if(left && !left.walls[1] && !visited_nodes_for_path.find(n => n.x === left.x  - (size / 2) && n.y === left.y - (size / 2))){
        console.log("left")
        ...
        if(left_x === end_node.x && left_y === end_node.y){
            ...
            let left_block = new Block(left_x,left_y, node)
